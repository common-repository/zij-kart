...
<div id="zijkart">
	<div class="heading">
		<?php echo __('Category form','zijkart'); ?>
		<a href="admin.php?page=zk_category"><?php echo __('Categories','zijkart'); ?></a>
	</div>
	<?php
		$form = zijkart::get_d('form');
		$html = new zijkartHTML();
	?>
	<form method="POST" action="admin.php?page=zk_category&zktask=save" enctype="multipart/form-data">
		<div class="row">
			<?php
		...
		<div class="row">
			<?php 
				...
				if($form && $form->photo != '')
					echo '<img src="'.zijkart::$uploadurl.'/'.zijkart::getOption('directory')."/categories/category_".$form->id."/".$form->photo.'" />'; 
			?>
		</div>
		<div class="row">
			<?php
		...
		<div class="row">
			<?php
				...
				echo $html->hidden('created',$form);
		 		echo $html->submit(__('Save Category','zijkart'),array('class'=>'button'));
			?>
		</div>
	</form>
</div>
